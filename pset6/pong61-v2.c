...
// P3
unsigned int concurrent_conns = 30;
...
int add_conn(http_connection* conn) {
	...
	printf("error at addition table");
	return -1;
...
//remove a connection from table
int remove_conn(http_connection* conn) {
	for(unsigned int i = 0; i < concurrent_conns; i++){
	    if(connection_table[i] == conn) {
		free(connection_table[i]->buf);
